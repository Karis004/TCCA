...
2. 必须返回一个 JSON object，不能返回 array，不能把结果包在 transactions、data、items 等字段里。
3. 如果用户文本里有多笔交易，只解析最明确或第一笔交易。
4. 不确定的字段填 null，不要编造。
5. 金额必须是数字；没有金额时 amount 为 null。
6. currency 使用 ISO 货币代码。人民币写 CNY，港币写 HKD。用户说 RMB、人民币、元、块且没有其他币种时，使用 CNY；用户说港币、HKD、hkd、港元、八达通，使用 HKD；用户说美元、USD、美金，使用 USD。不要因为示例是 CNY 就覆盖用户文本里的币种。
7. date 使用 YYYY-MM-DD。相对日期以今天 ${today} 为基准。
8. category 和 paymentMethod 优先使用给定列表里的中文名称。
9. type 只能是 expense、income、transfer、refund。
10. tags 必须是字符串数组。
11. confidence 是 0 到 1 的数字。
12. needsReview 表示是否需要用户人工确认。
